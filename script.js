...
async function useLocation(lat, lng) {
  ...
  const url = `/api/proxy/maps/api/place/nearbysearch/json?location=${lat},${lng}&radius=1500&type=cafe&key=${apiKey}`;
  try {
...
function displayCards(cafes) {
  ...
  cafes.forEach((cafe, i) => {
    ...
    const imgUrl = cafe.photos?.[0]?.photo_reference
      ? `/api/proxy/maps/api/place/photo?maxwidth=400&photoreference=${cafe.photos[0].photo_reference}&key=${apiKey}`
